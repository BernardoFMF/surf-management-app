...
const companyController = (data) => {
	...
	const updateCompany = asyncHandler(async (req, res) => {
		...
		const company = await services.updateCompanyServices(req.params.cid, req.body.nif, req.body.type, req.body.name, req.body.phone_number, req.body.postal_code, req.body.address, req.body.location, req.body.img, req.body.is_deleted, req.body.iban)
		if (company) res.json(company)
	})
	...
	const getMemberValidation = asyncHandler(async (req, res) => {
		...
		const member = await services.getMemberValidation(req.params.id, req.body.pin)
		if (member) {
			res.status(201)
			res.json(member)
		}
