...
import { deleteQuota} from '../../store/actions/quotaActions'
import DateInputField from '../../components/multiStepForm/DateInputField';
import { parse, isDate } from "date-fns";
import { QUOTA_DELETE_RESET } from '../../store/constants/quotaConstants'
...
const QuotaCreateDialog = ({open, closeHandler}) => {
    ...
    const create = useSelector((state) => state.deleteQuota)
    const { loading, error, quotaDelete: message } = create

    
    const handleSubmitDelete = async (values) => {
        let date = formatDate(values.date)
        dispatch(deleteQuota(date))
    }
    ...
    return (
        ...
        <DialogContent>
            { message && <Box sx={{ pl: { md: 2 }, pt: 2 }}><Alert severity="success" onClose={() => dispatch({ type: QUOTA_DELETE_RESET })}>{t(message.message_code)}</Alert></Box> }
            { error && <Box sx={{ pl: { md: 2 }, pt: 2 }}><Alert severity="error" onClose={() => dispatch({ type: QUOTA_DELETE_RESET })}>{t(error)}</Alert></Box> }
            <Box
                sx={{
                display: 'flex',
